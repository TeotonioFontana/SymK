...
def main(argv=None) -> int:
    ...
    ap.add_argument("--skip-ide-check", action="store_true", help="Allow scaffolding even if --dest is not a detected PyCharm project")
    ap.add_argument("--ide-report", action="store_true", help="Print PyCharm project diagnostics")
    ...
    # __init__.py files
    for p in [root / pkg, paths["common"], paths["boundaries"], paths["core"], paths["adapters"], paths["models"]]:
        touch(p / "__init__.py")
    if args.services:
    ...
    if args.init_git:
        try:
            subprocess.run(["git", "init"], cwd=str(root), check=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            subprocess.run(["git", "add", "-A"], cwd=str(root), check=False)
            subprocess.run(["git", "commit", "-m", "chore: initial SymK scaffold"], cwd=str(root), check=False)
